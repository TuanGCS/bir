...
void ip_putintable(ip_table_t * table, addr_ip_t ip, interface_t* interface) {
	...
	if (table->table_size == 0) {
		table->table_size++;
		table->entries = (ip_table_entry_t *) malloc(
				sizeof(ip_table_entry_t) * table->table_size);
	} else {
		table->table_size++;
		table->entries = (ip_table_entry_t *) realloc((void *) table->entries,
				sizeof(ip_table_entry_t) * table->table_size);
	}
...
void ip_onreceive(packet_info_t* pi, packet_ip4_t * ipv4) {
	...
	ip_table_entry_t * dest_ip_entry = ip_longestprefixmatch(
			&pi->router->ip_table, ipv4->dst_ip);
